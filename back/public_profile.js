function	checklike(user_id, like_id, callback)
{
	...
	var b = 0
	 con.query('SELECT * FROM likes WHERE user_id = ? AND like_id = ?', [user_id, like_id], function (err, rows) { if (err) throw err 
	 	...
	 	con.query('SELECT * FROM likes WHERE user_id = ? AND like_id = ?', [like_id, user_id], function (err, rows) { if (err) throw err 
	 	if (rows.length == 1)
	 		b = 1;
	 	if (user_id == like_id)
	 		return callback(-1);
...
checklike(req.session.profile.id, result[0].id, function(like) {
