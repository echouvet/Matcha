...
function newimg()
{
	...
	form.parse(req, function (err, field, files) { if (err) throw err;
		...
		{
			res.render('profile.ejs', {req: req, css: css, error: 'A server error occured', profile: req.session.profile});
			return ;
		}
		if (files.file.type !== 'image/png' && files.file.type !== 'image/jpeg' && files.file.type !== 'image/jpg')
		{
			res.render('profile.ejs', {req: req, css: css, error: 'Only jpeg and png image types aloud', profile: req.session.profile});
			return ;
		}
		if (files.file.size > 5000000)
		{
			res.render('profile.ejs', {req: req, css: css, error: 'Your image is too big', profile: req.session.profile});
			return ;
		}
		var i = 1
		while (i <= 5)
